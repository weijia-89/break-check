...
GitHub rules for the public skill repo **break-check**. Apply only after the remote exists and the operator confirms `GH_REPO`.

**Default target:** `weijia-89/break-check` (set explicitly; never assume from cwd alone).

## Policy

| Rule | Setting | Notes |
| ---- | ------- | ----- |
| Default branch | `main` | Matches current remote default. |
...
```bash
cd ~/Projects/break-check.skill
export GH_REPO=weijia-89/break-check
...
The script is idempotent: repeated `APPLY=1 DRY_RUN=0` runs send the same PUT payload. It refuses apply if `gh repo view` fails or `APPLY` is not set.

## Manual UI steps

1. Open `https://github.com/weijia-89/break-check/settings/branches`.
...
Apply (requires repo + admin + `APPLY=1`):
...
When CI exists (e.g. lint or skill-body validation), add a section here and extend the script payload:
...
- `README.md` — repo overview and skills.sh install path
