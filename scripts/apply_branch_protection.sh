...
# Requires GH_REPO=owner/name. Live apply requires APPLY=1 and DRY_RUN=0.
# Never stores tokens in this file.
set -euo pipefail

GH_REPO="${GH_REPO:-weijia-89/break-check}"
DRY_RUN="${DRY_RUN:-1}"
APPLY="${APPLY:-0}"
BRANCH="${BRANCH:-main}"
OWNER="${GH_REPO%%/*}"
...
echo "target: $GH_REPO branch=$BRANCH DRY_RUN=$DRY_RUN APPLY=$APPLY" >&2
...
if [[ "$APPLY" != "1" ]]; then
  echo "error: DRY_RUN=0 requires APPLY=1 (operator intent gate)." >&2
  exit 1
fi
